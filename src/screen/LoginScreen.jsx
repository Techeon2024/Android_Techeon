...
const LoginScreen = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    ...
    if (!emailOrPhone || !password) {
      setErrorMessage('Phone Number or Email and Password are required.');
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        // If login is successful, navigate to Resume screen
        setErrorMessage('');
        navigation.navigate('Resume');
      } else {
        // If there's an error, show the error message returned by the backend
